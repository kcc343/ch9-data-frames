...
most_popular_in_year <- function(year_value) {
  names_year <- names[names$year == year_value, ]
  popular_name <- names_year[names_year$prop == max(names_year$prop), "name"]
}

# What was the most popular female name in 1994?

female_1994 <- most_popular_in_year(1994)
